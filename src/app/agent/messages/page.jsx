  ...
  useEffect(() => {
    const loadConversations = async () => {
      ...
      const seed = filings.map((f) => ({
        ...
        client: f.applicantName || "Client",
        messages: [
          {
            id: `${f.referenceNumber}-seed`,
            sender: "client",
            text: `This is a placeholder conversation for ${f.referenceNumber || "your assignment"}. Live chat history requires messaging APIs.`,
          },
        ],
      }));
    ...
    <div className="-m-6 h-[calc(100vh-3.5rem)] flex bg-white">
      <div className="w-72 border-r border-gray-100 flex flex-col shrink-0">
        ...
        <p className="px-4 py-2 text-[11px] text-gray-400 border-b border-gray-100">
          Messaging endpoints are not available yet. This view uses assigned cases as conversation stubs.
        </p>
        <div className="p-3 border-b border-gray-100">
          <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2">
            <span className="material-symbols-outlined text-gray-400 text-base">search</span>
            <input
              placeholder="Filter by case ID or client..."
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.map((c) => (
            ...
            >
              ...
              <p className="text-[11px] text-gray-400">{c.client}</p>
            </button>
          ))}
          ...
          <>
            <div className="px-6 py-3.5 border-b border-gray-100 flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-[#0d1b2a] flex items-center justify-center text-white text-xs font-bold">
                {active.client?.charAt(0) || "C"}
              </div>
              <div className="flex-1">
                <p className="text-sm font-bold text-[#0d1b2a]">{active.client}</p>
      ...
      <div className="w-64 border-l border-gray-100 p-5 hidden xl:block overflow-y-auto shrink-0">
        ...
        <p className="text-xs text-gray-400">
          Client insights will populate once messaging and analytics endpoints are available.
        </p>
      </div>
    </div>
  );
