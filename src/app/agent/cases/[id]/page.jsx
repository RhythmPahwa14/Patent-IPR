...
import { getAgentPatents, getFilingByReference, updateAgentPatentStatus } from "@/lib/api";

const STATUS_STYLES = {
  ASSIGNED: "text-blue-700 border-blue-300 bg-blue-100",
...
export default function AgentCaseDetailPage() {
  ...
  useEffect(() => {
    const loadCase = async () => {
      ...
      const agentList = await getAgentPatents();
      if (agentList.ok) {
        const found = (agentList.items || []).find((p) => {
          const key = p.referenceNumber || p.patentId || p.id;
  ...
  const updateId = filing?.patentId || filing?.id || referenceNumber;
    ...
    return [
      {
        ...
        state: filing.status === "ASSIGNED" ? "active" : "done",
      },
      {
        label: "Under Review",
  ...
  const submitDecision = async (status) => {
    ...
    const result = await updateAgentPatentStatus(updateId, status);

    if (!result.ok) {
      setActionState({ submitting: false, message: result.data?.message || "Unable to update status.", type: "error" });
      return;
    }

    setFiling((prev) => prev ? { ...prev, status, updatedAt: new Date().toISOString() } : prev);
    setActionState({ submitting: false, message: `Marked as ${status}.`, type: "success" });
    setDecisionNote("");
  };
    ...
    <div className="max-w-5xl mx-auto space-y-5">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          ...
          <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 min-w-[240px]">
            <p className="text-[10px] text-gray-400 uppercase tracking-widest">Decision Panel</p>
            <p className="text-xs text-gray-500 mt-1">Log your review outcome for this case.</p>
