...
const openApiSpec = {
  ...
  info: {
    ...
    version: "1.0.0",
    description:
      "APIs for authentication and managing patent filings. Authorization header format is Bearer <token>.",
  },
  servers: [
    {
      url: "/",
  ...
  components: {
    securitySchemes: {
      bearerAuth: {
        ...
        bearerFormat: "JWT",
        description:
          "Paste only the JWT token value in Swagger Authorize dialog (without the 'Bearer ' prefix).",
      },
    },
    schemas: {
      MessageResponse: {
  ...
  paths: {
    ...
    "/api/auth/register": {
      post: {
        ...
        description: "Creates a CLIENT user; role input is ignored by backend.",
        requestBody: {
          required: true,
          content: {
            "application/json": {
